...
<xsl:param name="usertype">coordinator</xsl:param>
<xsl:param name="formaction">editcoordinator</xsl:param>
...
<xsl:template match="/">
  ...
  <p>
    ...
    <xsl:choose>
    <xsl:when test="substring($formaction,1,14) = 'addcoordinator'">
      <span class="pagetitle">Add Coordinator</span><br/>
	  <p class="subtitle">Please enter required data for each project coordinator listed on your proposal.</p> 
	  <p class="subtitle">You should also add or delete coordinator that have changed since submitting your proposal.</p>
    </xsl:when>
    <xsl:otherwise>
    <span class="pagetitle">Edit Coordinator</span><br/>
	<p class="subtitle">Please update data for each project coordinator listed on your proposal.</p>
	<p class="subtitle">You should also add or delete coordinator that have changed since submitting your proposal.</p>
	</xsl:otherwise>
    </xsl:choose>
  ...
  <form method="post" action="sare_main.aspx?do={$formaction}" name="coordinatorDetails">
...
<td>
 ...
 <tr>
   <td>*Zip Code</td>
   <td><input onkeydown="return numbersonly(event,false)" maxlength="5" size="5" name="addrZip" value="{contact/addrZip}" onkeypress="WarnUser()"/></td>
 </tr>
 <tr>
   <td>Zip +4</td>
   <td><input onkeydown="return numbersonly(event,false)" maxlength="4" size="4" name="addrZip4" value="{contact/addrZip4}" onkeypress="WarnUser()"/>
</td>
</tr>
<tr>
<td/>
...
<td>
   <input maxlength="16" size="16" name="numPhone" value="{contact/numPhone}" onkeypress="WarnUser()" onkeydown="javascript:backspacerDOWN(this,event);" onkeyup="javascript:backspacerUP(this,event);"/>
	<xsl:choose>
		<xsl:when test="contact/numPhone = '' and substring($formaction,1,14) != 'addcoordinator'"><br/>
			<input name="partPhone" type="checkbox" checked="checked"/>This coordinator does not have a phone
		</xsl:when>
		<xsl:otherwise>
			<br/><input name="partPhone" type="checkbox"/>This coordinator does not have a phone
		</xsl:otherwise>
	</xsl:choose>
</td>
</tr>
...
<td>Fax</td>
<td>
	<input maxlength="16" size="16" name="numFax" value="{contact/numFax}" onkeypress="WarnUser()" onkeydown="javascript:backspacerDOWN(this,event);" onkeyup="javascript:backspacerUP(this,event);"/>
</td>
</tr>
<tr>
<td>Cell</td>
<td>
	<input maxlength="16" size="16" name="numCell" value="{contact/numCell}" onkeypress="WarnUser()" onkeydown="javascript:backspacerDOWN(this,event);" onkeyup="javascript:backspacerUP(this,event);"/>
</td>
</tr>
<tr>
<td/>
...
<td>*Email Address</td>
<td>
	<input maxlength="255" size="50" name="email" value="{contact/email}" onkeypress="WarnUser()"/>
	<xsl:choose>
		<xsl:when test="contact/email = '' and substring($formaction,1,14) != 'addcoordinator'">
			<br/>
			<input name="partEmail" type="checkbox" checked="checked"/>This coordinator does not have an e-mail address
		</xsl:when>
		<xsl:otherwise>
			<br/><input name="partEmail" type="checkbox"/>This coordinator does not have an e-mail address
		</xsl:otherwise>
	</xsl:choose>
   </td>
</tr>
...
<td colspan="1">
	<xsl:if test="substring($formaction,1,14) != 'addcoordinator'">
      ...
      <tr>
		  <td>
			  <xsl:choose>
			  <xsl:when test="substring($formaction,1,14) = 'addcoordinator'">
				  <input type="submit" value="Save and Add Next Coordinator" name="cmdAddNextCoordinator" ID="cmdAddNextCoordinator" onclick="NoPrompt()"/>&#160;&#160;
				  <input type="submit" value="Save Coordinator" name="cmdChangeCoordinatorDetails" ID="cmdChangeCoordinatorDetails" onclick="NoPrompt()"/>
				  <br/>
			  </xsl:when>
			  <xsl:otherwise>
				  
					  <input type="submit" value="Delete Coordinator" name="cmdDeleteCoordinatorDetails" ID="cmdDeleteCoordinatorDetails" onclick="NoPrompt()"/>
					  <br/>
				  
				  <td>
					  <input type="submit" value="Update Coordinator" name="cmdChangeCoordinatorDetails" ID="cmdChangeCoordinatorDetails" onclick="NoPrompt()"/>
					  <br/>
				  </td>
			  </xsl:otherwise>
			  </xsl:choose>
		...
		<td>
			<input type="reset" value="Reset Form" name="cmdResetChangeCoordinatorDetails" ID="cmdResetChangeCoordinatorDetails" onclick="NoPrompt()"/><br/>
		</td>
		<td>
			<input type="submit" value="Cancel Changes" name="cmdCancelChangeCoordinatorDetails" ID="cmdCancelChangeCoordinatorDetails" onclick="NoPrompt()"/>
		</td>
		<td>

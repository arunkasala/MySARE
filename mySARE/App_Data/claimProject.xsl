...
<script LANGUAGE="JavaScript">
	...
	function isStateAbbreviation(claimProject)
	{
		var projectNum = claimProject.projNum.value;
		...
		window.alert(state);
		if (state.match(states) == null)
		{
			alert("This is a state report.");
			return false;
		}
		else
		{
...
<p>
	...
	<br/>
	<span class="pagetitle">MySARE: Retrieve a New Project</span>
	<br/>
	<p class="subtitle">Use MySARE to submit your project reports to the SARE program and share results with the general public. Copy and paste the project number and project code that you received via email into the fields below. Then click Retrieve Project. Retrieving a project gives you access to the MySARE system, where you will enter information and reports about your grant.</p>
	<xsl:if test="$message">
		<br/>
		<b>
			<i>
...
<form id="claimProject" name="claimProject" method="post">
    ...
    <tr>
      <td>
        <input name="Claim" type="submit" value="Retrieve Project"  onclick="return isStateAbbreviation(claimProject.projNum.value)"/>
      </td>
    </tr>
    </table>
  </form>
